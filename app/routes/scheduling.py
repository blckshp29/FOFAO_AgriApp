...
@router.post("/scheduling/corn/{field_id}")
def generate_corn_schedule(
    field_id: int,
    payload: CornScheduleRequest | None = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    payload = payload or CornScheduleRequest()
    field = db.query(Field).filter(
        Field.id == field_id,
        Field.owner_id == current_user.id
    ).first()
    ...
    if field.crop_type.value != "corn":
        raise HTTPException(status_code=400, detail="Corn schedule is only for corn fields")

    if payload.corn_type:
        field.corn_type = payload.corn_type
    if payload.crop_variety:
        field.crop_variety = payload.crop_variety
    ...
    try:
        tasks = scheduling_service.generate_corn_schedule(
            db=db,
            field=field,
            user_id=current_user.id,
    ...
    selected_profile = field.corn_type or field.crop_variety or "yellow"
    return {
        "message": f"Generated {len(tasks)} corn tasks for {selected_profile}",
        "field_id": field_id,
        "corn_type": field.corn_type,
